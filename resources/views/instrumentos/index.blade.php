...
@extends('layouts.app')
@section('contenido')
<h3 class="titulo">INSTRUMENTOS</h3>
<div class="d-md-flex justify-content-md-end">
    <form action="{{route('instrumentos.index')}}" method="GET">
        <div class="btn-group">
...
<div>
    <a href="{{ route('instrumentos.create') }}" class="btn-sm btn-primary">Agregar</a>
    <a href="{{ route('descargarPDFInstrumentos') }}" class="btn-sm btn-danger">PDF</a>
    &nbsp;
</div>
<br>
    ...
    <thead class="table table-striped table-bordered table-dark">
        <!--<thead class="table table-striped table-bordered">-->
            <td style="font-weight: bold;">ID</b></td>
        ...
        <!--<tbody class="table table-bordered">-->
            @foreach ($instrumentos as $instrumento)
                <tr>
                    ...
                    <td style="font-weight: bold;">{{$instrumento->nombre}}</td>
                    <td>{{$instrumento->descripcion}}</td>
                    <td>{{$instrumento->marca}}</td>
                    <td style="text-align: center;">{{$instrumento->cantidad}}</td>
                    <td>{{$instrumento->fecha_mantenimiento}}</td>
                    <td class="btn btn-group">
                        <form><a class="btn btn-primary" href="{{route('instrumentos.show',$instrumento)}}"><i class="bi bi-eye"></i></a></form>
                        <form><a class="btn btn-warning" href="{{route('instrumentos.edit',$instrumento)}}"><i class="bi bi-pencil-square"></i></a></form>
                        <form action="{{route('instrumentos.destroy', $instrumento)}}" method="POST" class="formEliminar">
                            @method('DELETE')
                            @csrf
        ...
        <tfoot>
            <tr>
                 <td colspan="4">{{$instrumentos->appends(['busqueda'=>$busqueda])}}</td>
            </tr>
        </tfoot>
    </table>
