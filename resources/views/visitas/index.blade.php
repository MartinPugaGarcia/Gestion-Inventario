...
<div>
    <a href="{{ route('descargarPDFVisitas') }}" class="btn-sm btn-danger">PDF</a>
    &nbsp;
</div>
<br>
    ...
    <thead class="table table-striped table-bordered table-dark">
        <!--<thead class="table table-striped table-bordered">-->
            ...
            <td style="font-weight: bold;">MOTIVO DE VISITA</td>
        ...
        <!--<tbody class="table table-bordered">-->
            ...
            <tr>
                <td>{{$visita->id}}</td>
                <td style="font-weight: bold;">{{$visita->ncontrol}}</td>
                <td>{{$visita->nombre}}</td>
                <td>{{$visita->motivo}}</td>
                <td>{{$visita->fecha_hora}}</td>
                <td class="btn-group ">
                    <form><a class="btn btn-primary" href="{{route('visitas.show',$visita)}}"><i class="bi bi-eye"></i></a></form>
                    <form><a class="btn btn-warning" href="{{route('visitas.edit',$visita)}}"><i class="bi bi-pencil-square"></i></a></form>
                    <form action="{{route('visitas.destroy', $visita)}}" method="POST" class="formEliminar">
                        @method('DELETE')
                        @csrf
        ...
        <tfoot>
            <tr>
                <td colspan="4">{{$visitas->appends(['busqueda'=>$busqueda])}}</td>
            </tr>
        </tfoot>
    </table>
