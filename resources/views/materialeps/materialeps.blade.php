...
<head>
    ...
    <title>PDF PRESTAMOS MATERIALES</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.4.1/dist/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
</head>
<body>
    <header>
        <p class="text-center">LISTA DE PRÉSTAMO DE MATERIALES</p>
    </header>
    <div class="container">
        ...
        <table class="table table-striped">
            ...
            <tbody>
                @foreach($materialeps as $materiale)
                <tr>
                    <th scope="row">{{ $materiale->alumno }}</th>
                    <th scope="row">{{ $materiale->nombre }}</th>
                    <th scope="row">{{ $materiale->cantidad }}</th>
                    <th scope="row">{{ $materiale->docente }}</th>
                    <th scope="row">{{ $materiale->descripcion }}</th>
                    <th scope="row">{{ $materiale->fecha }}</th>
                    <th scope="row">{{ $materiale->hora }}</th>
                    <th scope="row">{{ $materiale->estado }}</th>
                </tr>
                @endforeach
            </tbody>
        </table>
